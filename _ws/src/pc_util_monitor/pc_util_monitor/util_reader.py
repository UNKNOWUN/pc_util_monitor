...
class UtilSample:
    ...
    gpu_util : float
        GPU utilization in [0.0, 1.0], or NaN if unavailable.
    status : str
        Status string such as OK_CPU_GPU / NO_GPU / OK_CPU_ONLY.
...
def _is_wsl() -> bool:
    """Detect whether this environment is WSL."""
    try:
...
def _read_windows_cpu_percent_via_powershell() -> Optional[float]:
    ...
    -------
    Optional[float]
        CPU utilization percentage in [0.0, 100.0] on success. Returns None on
        failure.

    Notes
    -----
    Works when running under WSL and powershell.exe is accessible. Output may
    contain locale-specific decimal separators, so the result is parsed
    defensively.
...
def read_proc_stat_cpu_times() -> Tuple[int, int]:
    """
    Read the first 'cpu' line from /proc/stat and return (idle_all, total_all).

    idle_all is idle + iowait. total_all is the sum of all CPU time fields.
    """
...
class CpuUtilReader:
    ...
    not reflect host usage well. PowerShell provides a host-wide metric closer to
...
class NvidiaSmiUtilReader:
    """
    Read NVIDIA GPU utilization via nvidia-smi.

    If nvidia-smi is not available or fails, read_util returns None.
    """
